function scpcatchment_matrix(td)
...
disp('Assigning Basin Identification Numbers.');
...
%             pause(0.001);
            pst = pst +1;
            disp(strcat(num2str(percent),' Percent of assignment done'));
        end
...
disp(strcat('A total of .',num2str(length(netwk_order_ind(td).ind)),' basin(s) are extracted'));
